...
<body class="bg-cream font-sans text-gray-700 h-screen flex overflow-hidden">
    ...
    <aside class="w-[260px] bg-primary-dark flex flex-col shrink-0 text-white shadow-xl z-20">
        ...
        <nav class="flex-1 overflow-y-auto sidebar-scroll py-6 px-4 flex flex-col gap-1.5">
            ...
            <a href="{{ route('hama') }}" class="flex items-center gap-3 px-3 py-2.5 bg-primary-mid border-l-[3px] border-white text-white font-semibold rounded-r-lg transition-colors"><i class="ph ph-bug text-[20px]"></i><span class="text-[15px]">Pengendalian Hama</span></a>
    ...
    <main class="flex-1 flex flex-col min-w-0 bg-[#EEEEEE] overflow-y-auto p-10">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div class="col-span-1 lg:col-span-8 bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                {{-- FILTER CHIPS (TOMBOL KATEGORI) --}}
                <div class="flex flex-wrap items-center gap-3 mb-8" id="filter-container">
                    <button data-filter="Semua" class="filter-btn bg-primary-dark text-white px-5 py-2 rounded-full text-[13px] font-semibold shadow-sm transition">Semua</button>
                    <button data-filter="Berat" class="filter-btn bg-white border border-gray-200 text-gray-600 px-5 py-2 rounded-full text-[13px] font-semibold flex items-center gap-2 hover:bg-gray-50 transition">
                        <div class="w-2 h-2 rounded-full bg-red-500"></div> Berat
                    </button>
                    <button data-filter="Sedang" class="filter-btn bg-white border border-gray-200 text-gray-600 px-5 py-2 rounded-full text-[13px] font-semibold flex items-center gap-2 hover:bg-gray-50 transition">
                        <div class="w-2 h-2 rounded-full bg-yellow-500"></div> Sedang
                    </button>
                    <button data-filter="Ringan" class="filter-btn bg-white border border-gray-200 text-gray-600 px-5 py-2 rounded-full text-[13px] font-semibold flex items-center gap-2 hover:bg-gray-50 transition">
                    ...
                    @forelse($riwayats as $rw)
                        ...
                        <div class="riwayat-item border {{ $borderColor }} rounded-[12px] p-6 hover:shadow-sm transition bg-white relative" data-keparahan="{{ $rw->tingkat_keparahan }}">
                            ...
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <div class="flex gap-2 text-gray-300">
                                    ...
                                    <form action="{{ route('hama.destroy', $rw->id) }}" method="POST" class="inline form-delete-hama">
                                        @csrf @method('DELETE')
                                        <button type="button" class="hover:text-red-500 transition btn-hapus-hama"><i class="ph ph-trash text-lg"></i></button>
                                    </form>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-2 gap-y-6 gap-x-4 mb-6">
                                <div><p class="text-[11px] text-gray-400 mb-1 flex items-center gap-1.5"><i class="ph ph-flask"></i> Metode</p><p class="text-[13px] font-bold text-gray-800">{{ $rw->metode_pengendalian }}</p></div>
                                <div><p class="text-[11px] text-gray-400 mb-1 flex items-center gap-1.5"><i class="ph ph-test-tube"></i> Bahan</p><p class="text-[13px] font-bold text-gray-800">{{ $rw->bahan_pengendalian }}</p></div>
                                <div><p class="text-[11px] text-gray-400 mb-1 flex items-center gap-1.5"><i class="ph ph-drop"></i> Dosis</p><p class="text-[13px] font-bold text-gray-800">{{ $rw->dosis }} {{ $rw->satuan }}</p></div>
                                <div><p class="text-[11px] text-gray-400 mb-1 flex items-center gap-1.5"><i class="ph ph-money"></i> Harga Beli</p><p class="text-[13px] font-bold text-gray-800">Rp {{ number_format($rw->harga_beli, 0, ',', '.') }}</p></div>
                            </div>
                            
                            <div class="flex items-end justify-between mt-2">
                                <p class="text-[12px] text-gray-400 italic max-w-[70%]">{{ $rw->catatan ? '"'.$rw->catatan.'"' : '' }}</p>
                                <span class="bg-orange-50 text-orange-600 px-3 py-1.5 rounded text-[12px] font-bold whitespace-nowrap">Total: Rp {{ number_format($rw->total_biaya, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    @empty
